...
export function TaskCalendar({ tasks }: TaskCalendarProps) {
  ...
  const overdueTasks = tasks.filter(
    (task) => task.dueDate && task.dueDate < Date.now() && task.status !== "done",
  ).length
  ...
  const getDayStyle = (day: number) => {
    ...
    const hasOverdue = dayTasks.some((task) => task.dueDate && task.dueDate < Date.now() && task.status !== "done")
    const hasFuture = dayTasks.some((task) => task.dueDate && task.dueDate >= Date.now())
    ...
    <Card className="border-0 shadow-sm">
      ...
      <CardContent>
        <div className="flex items-center justify-between mb-6">
          <Button variant="ghost" size="sm" onClick={() => navigateMonth("prev")} className="rounded-full">
            <ChevronLeft className="w-4 h-4" />
          </Button>
          <h3 className="text-lg font-semibold text-gray-900">
            {monthNames[month]} {year}
          </h3>
          <Button variant="ghost" size="sm" onClick={() => navigateMonth("next")} className="rounded-full">
            ...
            >
              {day && (
                <>
                  <span>{day}</span>
                  {getDotColor(day) && (
                    <div className={`w-1.5 h-1.5 rounded-full ${getDotColor(day)} absolute bottom-1`} />
                  )}
                </>
              )}
            </div>
